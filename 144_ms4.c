...
void GroceryInventorySystem(void)
{
	...
	int iVal, rv,found;
	welcome();
	loadItems(items, DATAFILE, &rv);//loading items from file to array
	if (loadItems(items, DATAFILE, &rv) == 0)
	{
	...
	else
	{
		...
		do
		{
			...
			case 2:
				...
				break;
			case 3:
				adjustQuantity(items, rv, CHECKOUT);
				saveItems(items, DATAFILE, rv);
				if (saveItems(items, DATAFILE, rv) == 0)
				{
...
void listItems(const struct Item item[], int NoOfItems)
{
	...
	for (i = 0; i < NoOfItems; i++)
	{
		...
		char* ans[3];//asked friends and searched online on how to use this
		if (item[i].isTaxed == 1)
		{
			*ans = "Yes";//asked friends and searched online on how to use this
			Total = item[i].price*item[i].quantity*(1 + TAX);
		}
		else
		{
			*ans = "No";//asked friends and searched online on how to use this
...
void adjustQuantity(struct Item item[], int NoOfRecs, int stock)
{
	...
	else
	{
		...
		if (stock == STOCK)
		{
			...
			{
				item[i].quantity += val;// adding quantity given to previous quantity
				printf("--== Stocked! ==--\n");
			}
		}
		...
		else
		{
			...
			{
				item[i].quantity -= val;//subtracting quantity given from previous quantity
				printf("--== Checked out! ==--\n");
